...
| Method | Path | 功能 | 当前判断 |
| --- | --- | --- | --- |
| `POST` | `/api/reviews/pre-pr` | 创建 Pre-PR Review | 前端已接入；部分后端源码不可稳定读取 |
...
- Pre-PR 创建成功后会初始化 Gate；Finding 人工状态变化、人工 Gate 决策和手动 CI 发布均以持久化 Gate 为主。
...
| Method | Path | 功能 |
| --- | --- | --- |
| `GET` | `/api/integration/ci-config` | 查询 CI 配置 |
| `PUT` | `/api/integration/ci-config` | 保存 CI 配置 |
...
集成链路的设计已经比较清楚：配置、执行、日志、失败重试、动作追踪都有对应对象。后续重点是更多 Provider、权限边界、密钥管理和真实生产环境验证。
...
运营中心目前能发现问题、排序问题、给建议，但还不能把问题变成完整任务：
