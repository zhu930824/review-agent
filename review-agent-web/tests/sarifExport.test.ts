...
test('review detail can post summary back to a pull request', () => {
  ...
  assert.match(detailView, /post<SummaryPublishResult>\('\/integration\/pr-summary\/comment'/)
  assert.match(detailView, /pullNumber:\s*Number\(prSummaryPullNumber\.value\)/)
  assert.match(detailView, /\/integration\/gitlab\/merge-requests\/summary-note/)
  assert.match(detailView, /summaryProvider/)
  assert.match(detailView, /mergeRequestIid/)
